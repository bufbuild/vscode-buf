...
export const restartBuf = new Command("buf.restart", CommandType.COMMAND_EXTENSION, (ctx, bufCtx) => {
  const callback = async () => {
    if (bufCtx.client) {
      log.info(`Request to stop language server (enabled: ${config.get<boolean>("enable")})`);
    ...
    bufCtx.client = new lsp.LanguageClient("Buf Language Server", serverOptions, clientOptions);
