...
export const updateBuf = new Command("buf.update", CommandType.COMMAND_SETUP, (ctx, bufCtx) => {
  return async () => {
    if (!bufCtx.buf) {
    ...
    log.info(`buf update: available=${upgrade.upgrade} installed=${upgrade.old}`);
    // Bail out if the new version is better or comparable.
    if (!upgrade.upgrade) {
      vscode.window.showInformationMessage(`buf is up-to-date (you have ${upgrade.old}, latest is ${upgrade.new})`);
      return;
    }
